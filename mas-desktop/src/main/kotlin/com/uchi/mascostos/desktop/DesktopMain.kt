...
import com.uchi.mascostos.shared.ui.RuntimeConfigLoader
import com.uchi.mascostos.shared.ui.RuntimeMode
...
class DesktopMain : Application() {
    override fun start(stage: Stage) {
        ...
        val runtimeConfig = RuntimeConfigLoader.loadDesktopConfig()

        val services = Bootstrap.bootWithSqlite(
            sqliteFile = runtimeConfig.sqlitePath,
            schemaScript = runtimeConfig.schemaPath,
        )
        ...
        val runtimeLabel = Label(
            if (runtimeConfig.mode == RuntimeMode.REMOTE) {
                "Configuración: REMOTE solicitada, ejecutando LOCAL SQLite en Desktop (${runtimeConfig.sqlitePath})"
            } else {
                "Configuración: LOCAL SQLite (${runtimeConfig.sqlitePath})"
            },
        )
        ...
        val projectsTab = Tab("1) Proyectos").apply {
            content = VBox(
        ...
        val catalogTab = Tab("2) Catálogo").apply {
            content = VBox(
                MasDesignSystem.Spacing.section,
                budgetForm,
        ...
        val itemsTab = Tab("3) Ítems y reportes").apply {
            content = VBox(
                MasDesignSystem.Spacing.section,
        ...
        val content = TabPane(projectsTab, catalogTab, itemsTab).apply {
